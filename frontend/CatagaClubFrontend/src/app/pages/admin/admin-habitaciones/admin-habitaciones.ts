...
export class AdminHabitaciones implements OnInit {
  ...
  obtenerHabitaciones() {
    this.apiAdmin.getHabitaciones().subscribe({
      ...
      error: (err: any) => console.error('Error al cargar habitaciones:', err)
    });
  }
  ...
  registrarCheckIn(id: number) {
    ...
    this.apiAdmin.actualizarHabitacion(id, {
      esta_ocupada: true,
      ...
      error: (err: any) => alert('Error al procesar el ingreso.')
    });
  }
  ...
  liberarHabitacion(id: number) {
    ...
    this.apiAdmin.actualizarHabitacion(id, {
      ...
      error: (err: any) => alert('Error al liberar la habitación.')
    });
  }
}